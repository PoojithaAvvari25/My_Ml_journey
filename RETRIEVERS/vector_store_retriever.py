...
vector_store = Chroma.from_documents(
    ...
    collection_name="my_croma_vector_store"
)

#convert vector store into retriever
retriever =vector_store.as_retriever(search_kwargs ={'k':2})

query = "tell me about a forest"
...
#without using retriever -directly using vector store

results_without_retriever = vector_store.similarity_search(query,k=2)#basic search
for i,doc in enumerate(results_without_retriever):
    print(f"\n---Result {i+1}---")
    print(f"Content:\n{doc.page_content}...")

#without retriever using vector store we can only search using a single strategy..but using retrievers we can perform search operation using different different search strategies-Advance search strategies